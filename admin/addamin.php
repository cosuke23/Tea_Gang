...
session_start();
	...
	$password=$_POST['password'];
		...
		$query=mysql_query("select * from admin where Email=email or Username=$username ");
		$num=mysql_num_rows($query);
		if($num>=1){
			echo"<script>alert('Email/Username already in use');</script>";
			//header("location:signup.php");
		}else{
		mysql_query("insert into admin(Name, Email, Contact, Password, Username)
...
<div class="container">
              <div class="row">
                <form role="form" method="POST" action="addamin.php" >
