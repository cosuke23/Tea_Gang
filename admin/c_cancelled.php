...
<!-- CONTENT START -->
    <div class="page-title-container">
     ...
     <h1 class="dashboard">Customized Delivered</h1>
...
$cus_name=$_SESSION['Name'];
          ...
          $result = mysql_query("SELECT cus_temp.*,customers.*,(cus_temp.cnt) as cnt,reservation.* FROM cus_temp left join customers  on customers.id=cus_temp.id left join reservation on customers.Name=reservation.firstname  where cus_temp.status='Cancelled' ");
          while($row = mysql_fetch_array($result))
            {
              echo '<tr class="record" id="'.$row['status'].'">';
          echo '<td>'.$row['Name'].' '.$row['Lname'].'</td>';
