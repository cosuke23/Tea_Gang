...
session_start();
		...
		while($row = mysql_fetch_array($result))
			...
			//	echo 'Delivery Type: '.$row['delivery_type'].'<br>';
				...
				echo 'Note : Outside Manila additional charge 100.00 </p> ';
...
session_start();
                                    ...
                                    $query=mysql_query("SELECT * from cus_temp where cus_name = '$cusname' and cnt='$cnt' ");
...
$querys=mysql_query("SELECT price from cus_temp where id = '".$_SESSION['id']."' and cnt='$cnt' ");
while($rows=mysql_fetch_array($querys)) {
	
 ?>
 <td> TOTAL PRICE : P<?php echo number_format($rows['price']); ?>.00 </td>
                                     <?php } ?>
	...
	?>	
	<?php 
mysql_query("UPDATE cus_temp set status='Pending'");
	?>
                               
</table>
</br>
<?php
echo 'Note : Payment should be made within 3 days else Cancelled </p> ';
?>
</div><br><br>
