...
<br>       
  ...
  $current_url = "lavander.php";
    
  $results = $mysqli->query("SELECT * FROM lavander WHERE qty>0");
$type="lavander";    
    if ($results) { 
...
$results = $mysqli->query("SELECT * FROM temp where id = '".$_SESSION['id']."'");
    if ($results) { 
          $url = base64_encode($url="http://localhost/htdocs/MCJ%20final/customer/lavander.php");
   $current_url="lavander.php";
        //fetch results set as object and output HTML
        while($obj = $results->fetch_object())
        {
