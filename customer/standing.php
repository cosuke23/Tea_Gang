...
<br>
      <ul class="grid cs-style-4">
        <center>
 <div style="font-size:35px; font-family:Trajan Pro; margin-left:80px; margin-top:18px; color:#FFCC66;">Standing Arrangement</div>       
   </center>
<br>      
    <?php
    //current URL of the Page. cart_update.php redirects back to this URL
 $current_url ="standing.php";
     
  $results = $mysqli->query("SELECT * FROM standing where qty>0");
$type="standing";    
    if ($results) { 
...
<div class="shopping-cart "   style="background:url('images/ft1.jpg')">
  <div  style='overflow-x:hidden;overflow-y:scroll;width:370px;height:400px'>
    <div style='width:340px;height:100px'><br>
<h2><center>Your Shopping Cart</center></h2><br>
<?php
    $total = 0;
$results = $mysqli->query("SELECT * FROM temp where id = '".$_SESSION['id']."'");
    if ($results) { 
          $url = base64_encode($url="http://localhost/htdocs/MCJ%20finals/customer/standing.php");
   $current_url="standing.php";
        //fetch results set as object and output HTML
        while($obj = $results->fetch_object())
        {
