...
<br>
      <ul class="grid cs-style-4">
        <center>
 <div style="font-size:35px; font-family:Trajan Pro; margin-left:80px; margin-top:18px; color:#FFCC66;">SunFlower</div>       
   </center>
<br>      
    <?php
    //current URL of the Page. cart_update.php redirects back to this URL
  $current_url = "sunflower.php";  
  $results = $mysqli->query("SELECT * FROM sunflower WHERE qty>0");
    if ($results) { 
  $type="sunflower"; 
...
$results = $mysqli->query("SELECT * FROM temp where id = '".$_SESSION['id']."'");
    if ($results) { 
          $url = base64_encode($url="http://localhost/htdocs/MCJ%20final/customer/sunflower.php");
   $current_url="sunflower.php";
        //fetch results set as object and output HTML
        while($obj = $results->fetch_object())
        {
