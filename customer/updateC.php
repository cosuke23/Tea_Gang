...
if(isset($_GET["emptycart"]) && $_GET["emptycart"]==1)
{
  $return_url = base64_decode($_GET["return_url"]); //return url
    $results = $mysqli->query("DELETE  FROM temp where id='$session_id'")or die(mysqli_error());
...
if(isset($_POST["type"]) && $_POST["type"]=='add')
{
  ...
  $product_qty  = filter_var($_POST["qty"], FILTER_SANITIZE_STRING);
...
$sqlad = mysqli_query($conn,"SELECT * FROM customers Where id='$session_id'");
  ...
  while($rowad= mysqli_fetch_array($sqlad)){ 
       ...
       $c_name= $rowad["Name"];
       $total=$obj->price * $product_qty;
     }
  $resultx = $mysqli->query("INSERT INTO `temp`(`id`, `flower_id`, `img`, `name`, `price`, `qty`, `total`, `cus_name`, `type`) VALUES ('$session_id','$product_code','$obj->img','$obj->name','$obj->price','$product_qty','$total','$c_name','Milk Tea')")or die(mysqli_error());
