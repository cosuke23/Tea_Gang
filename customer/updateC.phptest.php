...
if(isset($_POST["type"]) && $_POST["type"]=='add')
{
  ...
  $results = $mysqli->query("SELECT name,price,img FROM $type WHERE tea_id='$product_code'   LIMIT 1");
   $resultx = $mysqli->query($conn,"INSERT INTO `temp`(`cnt`, `id`, `flower_id`, `img`, `name`, `price`, `qty`, `total`, `cus_name`, `type`) VALUES ('','','','','','','','','',''"));
    ...
    if(isset($_SESSION["products"])) //if we have the session
    {
      ...
      foreach ($_SESSION["products"] as $cart_itm) //loop through session array
      {
        if($cart_itm["name"] == $Pname){ //the item exist in array
          ...
          $results = $mysqli->query("INSERT INTO `temp`(`cnt`, `id`, `flower_id`, `img`, `name`, `price`, `qty`, `total`, `cus_name`, `type`) VALUES ('','','','','','','','','',''")or die(mysqli_error());
          $found = true;
        }else{
