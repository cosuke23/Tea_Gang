...
<br>
      <ul class="grid cs-style-4">
        <center>
 <div style="font-size:35px; font-family:Trajan Pro; margin-left:80px; margin-top:18px; color:#FFCC66;">Vase Arrangement</div>       
   </center>
<br>      
    <?php
    //current URL of the Page. cart_update.php redirects back to this URL
  $current_url = "vase.php";
    
  $results = $mysqli->query("SELECT * FROM vase WHERE qty>0");
$type="vase";    
    if ($results) { 
...
$results = $mysqli->query("SELECT * FROM temp where id = '".$_SESSION['id']."'");
    if ($results) { 
          $url = base64_encode($url="http://localhost/htdocs/MCJ%20finals/customer/vase.php");
   $current_url="vase.php";
        //fetch results set as object and output HTML
        while($obj = $results->fetch_object())
        {
